...
def main():
    ap = argparse.ArgumentParser(description="타사 약관 PDF → 조항 인덱싱(JSONL)")
    ...
    ap.add_argument("--pdf-dir", required=True, help="원문 PDF가 있는 폴더")
    ...
    out = args.out
    if not out:
        out = f"peer_data/index/{args.insurer_code}/{args.product_group}/documents.jsonl"
